...
class HomeScreenv3 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Chambres visibles (doivent correspondre aux rooms utilisés par les réservations pour s'afficher)
    final rooms = [
      "101",
      "102",
      "103",
                ...
                showDialog(
                  ...
                  builder: (context) => RoomGeneratorDialog(
                    onRoomsGenerated: (generatedRooms) {
                      // Utiliser les chambres générées
...
class _CalendarTableWithDraggingState extends State<CalendarTableWithDragging> {
  ...
  Color _weekdayBgColor(DateTime d, {bool forHeader = false}) {
    // Jeudi: transparent (pas de couleur spéciale)
                  ...
                  child: Stack(
                    children: [
                      ...
                      ...widget.reservations.map((res) {
                        ...
                        // Positionnement correct: gauche = startIndex * dayWidth (PAS +1)
                        final double left = (startIndex + 1) * dayWidth;
                        // Largeur inclusive (on couvre le jour de fin)
                        final double width =
...
class _RoomGeneratorDialogState extends State<RoomGeneratorDialog> {
  ...
  void _generateRooms() {
    ...
    final int firstRoom = int.parse(_firstRoomController.text);
    final int lastRoom = int.parse(_lastRoomController.text);
    final String excludeText = _excludeRoomsController.text.trim();

    // Parse les numéros à exclure
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                // Premier numéro de chambre
                TextFormField(
                  controller: _firstRoomController,
                  decoration: const InputDecoration(
                    labelText: 'Premier numéro de chambre (par étage)',
                    hintText: 'Ex: 01',
                  ...
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                      return 'Veuillez entrer le premier numéro';
                    }
                    final number = int.tryParse(value);
                    if (number == null || number < 1 || number > 99) {
                      return 'Entre 01 et 99';
                    }
                    return null;
                  },
                ),
                ...
                // Dernier numéro de chambre
                TextFormField(
                  controller: _lastRoomController,
                  decoration: const InputDecoration(
                    labelText: 'Dernier numéro de chambre (par étage)',
                    hintText: 'Ex: 08',
                  ...
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                      return 'Veuillez entrer le dernier numéro';
                    }
                    final number = int.tryParse(value);
                    final firstNum =
                        int.tryParse(_firstRoomController.text) ?? 1;
                    if (number == null || number < firstNum || number > 99) {
                      return 'Doit être >= au premier numéro et <= 99';
                    }
                    return null;
                  },
                ),
                  ...
                  Container(
                    ...
                    child: SingleChildScrollView(
                      child: Wrap(
                        spacing: 8,
                        runSpacing: 4,
                        children: _generatedRooms.map((room) {
                          return Container(
...
// Fonction utilitaire pour afficher la dialog

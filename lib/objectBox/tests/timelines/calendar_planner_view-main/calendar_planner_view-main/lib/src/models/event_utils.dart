...
class EventUtils {
  ...
  static int countEventsForDay(DateTime day, List<CalendarEvent> events) {
    return events
        .where((event) => CalendarDateUtils.isSameDay(event.startTime, day))
        .length;
  }
  ...
  static Widget buildEventDots({
    ...
    final boxShape =
        shape == EventDotShape.circle ? BoxShape.circle : BoxShape.rectangle;
        ...
        children: List.generate(
          ...
          (i) => Container(
            ...
            decoration: BoxDecoration(
              color: color,
              shape: boxShape,
              borderRadius: shape == EventDotShape.diamond
  ...
  /// falls on that date.
  static List<CalendarEvent> filterEventsForDate(
      DateTime date, List<CalendarEvent> events) {
    return events
        .where((event) => CalendarDateUtils.isSameDay(event.startTime, date))
        .toList();
  }
  ...
  static bool doesEventOverlap(
      CalendarEvent event, List<CalendarEvent> events) {
    return events.any((otherEvent) {
      if (event == otherEvent) return false;
      return event.startTime.isBefore(otherEvent.endTime) &&
          event.endTime.isAfter(otherEvent.startTime);
    });
  }
  ...
  static List<CalendarEvent> getOverlappingEvents(
      CalendarEvent event, List<CalendarEvent> events) {
  ...
  static List<CalendarEvent> filterEventsForColumn(
      String columnId, List<CalendarEvent> events) {

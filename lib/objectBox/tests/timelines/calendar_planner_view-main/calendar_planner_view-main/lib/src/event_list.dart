...
class EventList extends StatelessWidget {
          ...
          child: SizedBox(
            ...
            child: Stack(
              children: [
                ...
                // Events
                ...events.map((event) {
                  if (event.startTime.isBefore(startTime) ||
                      event.endTime.isAfter(endTime)) {
                    return const SizedBox.shrink();
                  }

                  final startMinutes = (event.startTime.hour * 60 +
                          event.startTime.minute -
                          startHour * 60)
                      .toDouble();
                  final endMinutes = (event.endTime.hour * 60 +
                  ...
                  if (columns.isNotEmpty) {
                    if (event.columnId != null) {
                      columnIndex =
                          columns.indexWhere((col) => col.id == event.columnId);
                      if (columnIndex == -1) columnIndex = 0;
                    }
                  }

                  // Calculate overlapping events within the same column
                  final columnEvents = eventsByColumn[columns.isNotEmpty
                      ? columns[columnIndex].id
                      : 'default']!;
                  final overlapInfo = _calculateOverlappingEvents(
                          columnEvents, startTime, endTime)[event] ??
                      (false, 0, 1);
                  ...
                  final sameStartTimeEvents = columnEvents
  ...
  Map<CalendarEvent, (bool, int, int)> _calculateOverlappingEvents(
    ...
    final sortedEvents = events
        .where((event) =>
            event.startTime.year == startTime.year &&
            event.startTime.month == startTime.month &&
            event.startTime.day == startTime.day)
        .toList()

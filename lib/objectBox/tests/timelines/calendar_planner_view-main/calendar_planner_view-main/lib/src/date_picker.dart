...
class FlexibleDatePicker extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const maxDragDistance =
        150.0; // Maximum distance the calendar can be dragged
    ...
    // Event dots builder
    Widget Function(BuildContext, DateTime, DateTime) eventDotsBuilder =
    ...
    final defaultCalendarStyle = CalendarStyleUtils.getDefaultCalendarStyle(
      ...
      rangeHighlightColor:
          rangeHighlightColor ?? theme.colorScheme.primary.withAlpha(26),
    ...
    return GestureDetector(
      ...
      onVerticalDragUpdate: (details) {
        if (dragStartY.value != null) {
          ...
          if ((calendarFormat.value == CalendarFormat.month && delta > 0) ||
              (calendarFormat.value == CalendarFormat.week && delta < 0)) {
            // Apply damping and limit maximum drag distance
            final dampedDelta = delta * dragDamping;
            dragDistance.value = delta > 0
                ? dampedDelta.clamp(0, maxDragDistance)
                : dampedDelta.clamp(-maxDragDistance, 0);
          }
        }
      },
      onVerticalDragEnd: (details) {
        if (calendarFormat.value == CalendarFormat.month &&
            dragDistance.value > dragThreshold) {
          // Switch to week view
        ...
        builder: (context, child) {
          return Transform.translate(
            offset: Offset(
                0,
                -dragDistance.value *
                    0.3), // Reduced movement multiplier for more control
            child: Container(
              ...
              child: TableCalendar(
                ...
                selectedDayPredicate: (day) =>
                    CalendarDateUtils.isSameDay(selectedDate, day),
                ...
                calendarBuilders: CalendarBuilders(
                  markerBuilder: (context, day, events) =>
                      eventDotsBuilder(context, day, day),
                  headerTitleBuilder: (context, day) => Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(
                        CalendarDateUtils.getMonthName(day,
                  ...
                  dowBuilder: (context, day) => Center(
                    child: Text(
                      CalendarDateUtils.getWeekdayName(day,

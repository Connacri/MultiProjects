...
class _RoomFormScreenState extends State<RoomFormScreen> {
  ...
  late double _basePrice;
  late String _status;
  RoomCategory? _selectedCategory; // Catégorie sélectionnée
  ...
  void initState() {
    ...
    if (widget.room != null) {
      ...
      _basePrice = widget.room!.basePrice!;
      _status = widget.room!.status;
      _selectedCategory = widget.room!.category.target; // Catégorie actuelle
    } else {
      _code = '';
      _type = 'Single';
      _capacity = 1;
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              // Champ pour le code de la chambre
              TextFormField(
                initialValue: _code,
                decoration:
              ...
              // Champ pour le type de chambre (optionnel)
              DropdownButtonFormField<String>(
                value: _type,
              ...
              DropdownButtonFormField<String>(
                ...
                items:
                    ['Disponible', 'Occupée', 'En maintenance', 'Hors service']
              ...
              Consumer<HotelProvider>(
                builder: (context, provider, child) {
                  final categories = provider.roomCategories;
                  return DropdownButtonFormField<RoomCategory>(
              ...
              ElevatedButton(
                onPressed: () async {
                  if (_formKey.currentState!.validate()) {
                    ...
                    // Créer l'objet Room sans la catégorie dans le constructeur
                    final room = Room(
                      code: _code,
                      type: _type,
                      capacity: _capacity,
                      ...
                      ..id =
                          widget.room?.id ?? 0 // Définir l'ID après la création
                      ..category.target =
                          _selectedCategory; // Assigner la catégorie

                    if (widget.room == null) {
                      // Ajouter une nouvelle chambre
                      final roomId = await provider.addRoom(room);

                      // Si une catégorie est sélectionnée, l'assigner à la chambre
                      if (_selectedCategory != null) {
                        final newRoom =
